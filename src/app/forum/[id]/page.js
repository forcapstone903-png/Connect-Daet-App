  ...
  return (
    <main className="min-h-screen bg-[#f3f5f9] text-slate-900">
      {showAuthModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 text-center shadow-2xl">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><MessageSquare className="h-6 w-6" /></div>
            <h2 className="mt-4 text-xl font-black text-slate-900">Join the travel conversation</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">Create a free account to read the full discussion, reply, and follow helpful local advice.</p>
      ...
      <div className="mx-auto max-w-3xl px-3 py-6 sm:px-4 lg:px-6">
        ...
        {!userId && replies.length > 3 && (
          <button type="button" onClick={() => setShowAuthModal(true)} className="mb-6 flex w-full items-center justify-between rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-left transition hover:bg-emerald-100">
            <span><span className="block text-sm font-bold text-emerald-900">There&apos;s more helpful advice in this discussion</span><span className="mt-1 block text-xs text-emerald-700">Sign up to read all {replies.length} replies.</span></span>
            <span className="text-xs font-bold text-emerald-800">Read more →</span>
          </button>
        )}
        ...
        {thread.status !== 'locked' && thread.status !== 'archived' && (
          <div className="mb-6 rounded-[20px] border border-slate-200 bg-white p-5">
            ...
            <textarea
              ...
              onFocus={() => { if (!userId) setShowAuthModal(true) }}
            ...
            <div className="mt-4 flex gap-2">
              <button
                type="button"
                onClick={() => { if (!userId) setShowAuthModal(true); else handleReplySubmit() }}
            ...
            replies.slice(0, userId ? replies.length : 3).map((reply) => (
              <div key={reply.id} className={`rounded-[20px] border ${reply.is_best_answer ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-white'} p-4`}>
                ...
                <div className="mt-4 flex items-center gap-3 text-xs">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2 py-1 font-semibold text-emerald-700">▲ {reply.upvotes ?? reply.likes ?? 0}</span>
                  <span className="inline-flex items-center gap-1 rounded-lg bg-slate-50 px-2 py-1 font-semibold text-slate-500">▼ {reply.downvotes ?? 0}</span>
