...
export default function OnboardingPage() {
  ...
  const canContinue = step === 1 ? selectedTopics.length === 3 : step === 2 ? selectedPlaces.length === 3 : true
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_#ecfeff_0%,_#f8fafc_35%,_#f1f5f9_100%)] px-3 py-6 text-slate-900 sm:px-6 sm:py-10">
      <div className="mx-auto max-w-2xl">
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-8">
          {step === 1 && <><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-sky-700">Make your feed yours</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">Choose 3 favorite topics</h1><p className="mt-2 text-sm leading-6 text-slate-500">We’ll use these to shape your Daet community feed.</p><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">{topics.map((topic) => <button key={topic.id} type="button" onClick={() => toggleSelection(topic.name, selectedTopics, setSelectedTopics, 3)} className={`rounded-2xl border p-4 text-left transition ${selectedTopics.includes(topic.name) ? 'border-sky-500 bg-sky-50 ring-2 ring-sky-100' : 'border-slate-200 bg-slate-50 hover:border-sky-300'}`}><span className="text-xl">{topic.icon_emoji || '✦'}</span><span className="mt-2 block text-sm font-bold">{topic.name}</span>{selectedTopics.includes(topic.name) && <Check className="mt-2 h-4 w-4 text-sky-700" />}</button>)}</div><p className="mt-4 text-xs text-slate-500">{selectedTopics.length} of 3 selected</p></>}
          {step === 2 && <><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-700">Discover local favorites</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">Choose 3 places in Daet</h1><p className="mt-2 text-sm leading-6 text-slate-500">Pick the places you want to see more often.</p><div className="mt-6 space-y-2">{places.map((place) => <button key={place.id} type="button" onClick={() => toggleSelection(place.name, selectedPlaces, setSelectedPlaces, 3)} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left transition ${selectedPlaces.includes(place.name) ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-slate-50 hover:border-emerald-300'}`}><span className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl bg-emerald-100 text-emerald-700">{place.featured_image ? <img src={place.featured_image} alt="" className="h-full w-full object-cover" /> : <MapPin className="h-4 w-4" />}</span><span className="flex-1"><span className="block text-sm font-bold">{place.name}</span><span className="block text-xs text-slate-500">{place.location || place.category || 'Daet destination'}</span></span>{selectedPlaces.includes(place.name) && <Check className="h-4 w-4 text-emerald-700" />}</button>)}</div><p className="mt-4 text-xs text-slate-500">{selectedPlaces.length} of 3 selected</p></>}
          ...
          {step === 4 && <><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-violet-700">Make it yours</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">Add a profile photo and cover</h1><p className="mt-2 text-sm leading-6 text-slate-500">Both are optional. You can add them later from Edit profile.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center gap-2 text-sm font-bold"><ImagePlus className="h-4 w-4 text-sky-600" />Profile photo</div><MediaUpload bucket="profile-media" folder={`users/${userId}`} mediaType="image" existingMediaUrl={avatarUrl} onUploadComplete={setAvatarUrl} onUploadError={setError} buttonText="Add photo" maxSizeMB={5} /></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center gap-2 text-sm font-bold"><ImagePlus className="h-4 w-4 text-emerald-600" />Cover photo</div><MediaUpload bucket="profile-media" folder={`covers/${userId}`} mediaType="image" existingMediaUrl={coverUrl} onUploadComplete={setCoverUrl} onUploadError={setError} buttonText="Add cover" maxSizeMB={8} /></div></div><button type="button" onClick={completeOnboarding} className="mt-4 text-sm font-bold text-slate-500 hover:text-sky-700">Skip photos and finish</button></>}
