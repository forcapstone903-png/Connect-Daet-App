...
import { Search as SearchIcon, MapPin, CalendarDays, MessageSquare, Compass, Newspaper, X } from 'lucide-react'
...
function SearchContent() {
  ...
  return (
    <main className="min-h-screen bg-[#f3f5f9] text-slate-900">
      <div className="mx-auto max-w-[1000px] px-3 pb-10 pt-3 sm:px-4 lg:px-6">
        {/* Header */}
        <header className="sticky top-3 z-30 mb-6 rounded-[20px] border border-slate-200/80 bg-white/90 px-3 py-3 shadow-sm backdrop-blur md:px-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <form onSubmit={handleSubmit} className="hidden flex-1 items-center justify-center lg:flex">
          ...
          <div className="mt-3 lg:hidden">
            <form onSubmit={handleSubmit} className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-500">
              <SearchIcon className="h-4 w-4 flex-shrink-0" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
